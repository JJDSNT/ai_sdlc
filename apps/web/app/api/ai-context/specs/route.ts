...
export async function GET(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${baseUrl}/ai-context/specs?${search.toString()}`, {
      method: "GET",
      cache: "no-store",
    });
    ...
    return new NextResponse(text || JSON.stringify({ ok: true, specs: [] }), {
...
export async function POST(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(
      `${baseUrl}/ai-context/specs?repositoryRoot=${encodeURIComponent(repositoryRoot)}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body,
